...
def list_service_families():
    """List all available service families"""
    params = {}
    data = fetch_data(RETAIL_PRICES_URL)
    
    if not data or "Items" not in data:
        print("Error: Could not retrieve service families.")
        return []
    
    service_families = set()
    for service in data["Items"]:
        service_family = service.get("serviceFamily", "Unknown Service Family")
        service_families.add(service_family)

    print("\n===== Available Service Families =====")
    for family in service_families:
        print(f"- {family}")

    print(f"\nTotal Service Families found: {len(service_families)}\n")
    return list(service_families)
...
# Step 1: Select service family
service_families = list_service_families()
selected_family = input("Select a Service Family from the list above: ").strip()

# Step 2: Select region
region = input("Enter Azure region (e.g., us-east, west-europe): ").strip()

# Step 3: List available VM SKUs for the region
available_vms = list_vm_skus(region)
vm_sku = input("Enter VM SKU from the list above: ").strip()

# Step 4: List available Disk Types before selection
...
# Step 5: List available Bandwidth options before selection

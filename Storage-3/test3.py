...
def get_user_selection(options, prompt):
    ...
    while True:
        ...
        # Remove quotes from input if user mistakenly enters them
...
def extract_options(data):
    ...
    # Extract operation types dynamically (removing any accidental quotes)
    options["operations"] = sorted(
        [key.replace('"', '') for key in data["offers"].keys() if "operation" in key.lower()]
    )

    # Debugging: Print extracted operation keys
    print("\n=== Debugging: Extracted Operation Keys ===")
    for key in options["operations"]:
        print(key)
...
def calculate_cost(offers, offer_key_storage, offer_key_operations, region, capacity_gb, num_operations):
    ...
    operation_offer = offers.get(offer_key_operations.replace('"', ''), {})  # Ensure lookup matches API format
...
def main():
    ...
    operation_type = get_user_selection(options["operations"], "Operation Type")  # New Operation selection
    ...
    offer_key_operations = operation_type  # Using selected operation type
